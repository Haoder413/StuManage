...
export async function GET(request: NextRequest) {
  ...
  for (const subject of subjects) {
    if (subject.isActive) visible.set(subject.name, { id: subject.id, name: subject.name, source: "managed" });
  }
...
export async function POST(request: NextRequest) {
  ...
  const subject = await prisma.parentScheduleSubject.upsert({
    where: { workspaceId_parentId_name: { workspaceId: user.workspaceId, parentId: user.id, name } },
    create: { workspaceId: user.workspaceId, parentId: user.id, name, isActive: true },
...
export async function PATCH(request: NextRequest) {
  ...
  const subject = await prisma.$transaction(async (tx) => {
    await tx.parentScheduleSubject.upsert({
      where: { workspaceId_parentId_name: { workspaceId: user.workspaceId, parentId: user.id, name: oldName } },
      create: { workspaceId: user.workspaceId, parentId: user.id, name: oldName, isActive: false },
      update: { isActive: false },
    });
    const saved = await tx.parentScheduleSubject.upsert({
      where: { workspaceId_parentId_name: { workspaceId: user.workspaceId, parentId: user.id, name } },
      create: { workspaceId: user.workspaceId, parentId: user.id, name, isActive: true },
      update: { isActive: true },
    });
    await tx.parentScheduleItem.updateMany({
      where: { workspaceId: user.workspaceId, parentId: user.id, subjectLabel: oldName },
...
export async function DELETE(request: NextRequest) {
  ...
  await prisma.parentScheduleSubject.upsert({
    where: { workspaceId_parentId_name: { workspaceId: user.workspaceId, parentId: user.id, name } },
    create: { workspaceId: user.workspaceId, parentId: user.id, name, isActive: false },
    update: { isActive: false },
  });
